...
class Heap_frontier(Frontier):
    ...
    def push(self, state: State) -> None:
        self.counter += 1
        self.frontier.put((state.cost_with_heuristic, self.counter, state))

    def pop(self) -> State:
        return self.frontier.get()[2]
...
class Searcher:
    ...
    def get_child_states(self, state: State) -> Iterable[State]:
        ...
        for position in valid_moves:
            ...
            cost_with_heuristic = cost_to_reach + self.heuristic_function(position, self.goals)
            child_states.append(
                State(
                    position,
                    cost_to_reach,
                ...
                if child_state.king_position in self.visited_states:
                    ...
                    for goal in self.goals:
                        if goal == child_state.king_position:
                            return (self.backtrack(child_state), state.cost_to_reach)
...
def manhattan_heuristic(position: Position, goals: Iterable[Position]) -> float:
    return min([abs(goal[0] - position[0]) + abs(goal[1] - position[1]) for goal in goals])

def euclidian_heuristic(position: Position, goals: Iterable[Position]) -> float:
    return min([((goal[0] - position[0])**2 + (goal[1] - position[1])**2)**0.5 for goal in goals])

def empty_heuristic(position: Position, goals: Iterable[Position]) -> float:
    return 0

#############################################################################
######## Misc helper functions and thingthongs
#############################################################################
